...
def add_readings_to_database(oauth_session: OAuth2Session):
    if last_reading_date() == date.today():
        return

    for reading in weight_readings(oauth_session, last_reading_date(), date.today()):
        insert_reading(reading)


def weight_readings(oauth_session: OAuth2Session, range_start: date | None, range_end: date | None):
